...
import type { SSOConfig } from "@/domains/sso"
import { usePlatformSSOConfigs, useDeletePlatformSSOConfig } from "@/features/sso-admin/hooks/use-sso-configs"
import { SSOTestButton } from "@/features/sso-admin/ui/sso-test-button"

interface SSOConfigListProps {
  onCreateClick: () => void
  onEditClick: (config: SSOConfig) => void
}
...
export const SSOConfigList = ({
  onCreateClick,
  onEditClick,
}: SSOConfigListProps) => {
  const { data: configsRes, isLoading } = usePlatformSSOConfigs()
      ...
      <CardContent>
        {configs.length === 0 ? (
          <EmptyState
            ...
            description="Click &quot;Add Provider&quot; to set up SSO for the platform."
          />
        ) : (
          <div className="space-y-3">
            {configs.map((cfg: SSOConfig) => (
              <div
                key={cfg.id}
              ...
              >
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-medium">
                        {cfg.displayName || providerLabel(cfg.provider)}
                      </span>
                      <Badge variant={cfg.isEnabled ? "default" : "secondary"}>
                    ...
                    {cfg.allowedDomains.length > 0 && (
                      <p className="text-xs text-muted-foreground">
                        Domains: {cfg.allowedDomains.join(", ")}
                      </p>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    aria-label={`Edit ${cfg.displayName || providerLabel(cfg.provider)} configuration`}
                    title="Edit configuration"
                  >
                    <Pencil className="size-4" />
                  </Button>
                  <SSOTestButton configId={cfg.id} />
                  <ConfirmDialog
                    ...
                    details={[
                      {
                        label: "Provider",
                        value: cfg.displayName || providerLabel(cfg.provider),
                      },
                      ...(cfg.allowedDomains.length > 0
                        ? [
                            {
                  ...
                  >
                    <Button
                      ...
                      aria-label={`Delete ${cfg.displayName || providerLabel(cfg.provider)} configuration`}
                    >
