...
func TestGetSettings_OrgScoped(t *testing.T) {
	...
	db.Create(&persistent.Setting{OrgID: 1, Key: "python_poll_interval", Value: "5m"})
	db.Create(&persistent.Setting{OrgID: 1, Key: "npm_poll_interval", Value: "10m"})

	// Create settings for org 2
	db.Create(&persistent.Setting{OrgID: 2, Key: "python_poll_interval", Value: "30m"})
...
func TestUpdateSettings_OrgScoped(t *testing.T) {
	...
	var count int64
	db.Model(&persistent.Setting{}).Where("org_id = ?", 2).Count(&count)
...
func TestListAlerts_OrgScoped(t *testing.T) {
	...
	db.Create(&persistent.Alert{OrgID: 1, AnalysisID: analysis1.ID, PackageID: pkg1.ID, Severity: "critical", Status: "new", Message: "Alert for org 1"})
	db.Create(&persistent.Alert{OrgID: 2, AnalysisID: analysis2.ID, PackageID: pkg2.ID, Severity: "medium", Status: "new", Message: "Alert for org 2"})
...
func TestListAlerts_CrossOrg_ReturnsEmpty(t *testing.T) {
	...
	db.Create(&persistent.Alert{OrgID: 1, AnalysisID: analysis.ID, PackageID: pkg.ID, Severity: "critical", Status: "new", Message: "Org 1 only"})
...
func TestUpdateAlert_CrossOrg_Returns404(t *testing.T) {
	...
	alert := persistent.Alert{OrgID: 1, AnalysisID: analysis.ID, PackageID: pkg.ID, Severity: "critical", Status: "new", Message: "Org 1 alert"}
	db.Create(&alert)
...
func TestAlertOrgID_SetFromPackage(t *testing.T) {
	...
	pkg := persistent.Package{Name: "evil-pkg", Ecosystem: "python", OrgID: 42}
	db.Create(&pkg)
	rel1 := persistent.Release{PackageID: pkg.ID, Version: "1.0.0", Status: "completed"}
	db.Create(&rel1)
	rel2 := persistent.Release{PackageID: pkg.ID, Version: "1.1.0", Status: "completed"}
...
func TestPipelineProcessDiff_SetsAlertOrgID(t *testing.T) {
	...
	pkg := persistent.Package{Name: "backdoor-lib", Ecosystem: "npm", OrgID: 7}
	db.Create(&pkg)
...
func TestFullOrgIsolation(t *testing.T) {
	...
	// Set up org 1 data
	pkg1 := persistent.Package{Name: "safe-lib", Ecosystem: "python", OrgID: 1}
	db.Create(&pkg1)
	db.Create(&persistent.Setting{OrgID: 1, Key: "python_poll_interval", Value: "5m"})

	// Set up org 2 data
	pkg2 := persistent.Package{Name: "another-lib", Ecosystem: "npm", OrgID: 2}
	db.Create(&pkg2)
	db.Create(&persistent.Setting{OrgID: 2, Key: "python_poll_interval", Value: "20m"})
	...
	t.Run("alert isolation", func(t *testing.T) {
		...
		db.Create(&analysis)
		db.Create(&persistent.Alert{OrgID: 1, AnalysisID: analysis.ID, PackageID: pkg1.ID, Severity: "medium", Status: "new", Message: "test"})
...
func TestAppErrorTypes_Exist(t *testing.T) {
	...
	pkg := persistent.Package{OrgID: 1}
	assert.Equal(t, uint(1), pkg.OrgID)

	alert := persistent.Alert{OrgID: 1}
	assert.Equal(t, uint(1), alert.OrgID)

	setting := persistent.Setting{OrgID: 1}
	assert.Equal(t, uint(1), setting.OrgID)
}
