...
func TestResetPassword_ExpiredToken(t *testing.T) {
	...
	db.Model(&persistent.PasswordResetToken{}).
...
func TestVerifyEmail_ExpiredToken(t *testing.T) {
	...
	db.Model(&persistent.EmailVerificationToken{}).
		Where("user_id = ?", user.ID).
		Update("expires_at", time.Now().Add(-1*time.Hour))
...
func TestValidateAccessToken_WithPreviousSecret(t *testing.T) {
	...
	oldSvc := auth.NewService(userRepo, refreshTokenRepo, apiKeyRepo, passwordResetTokenRepo, emailVerificationTokenRepo, sessionRepo, nil, false, nil, oldSecret)
	...
	newSvc := auth.NewService(userRepo, refreshTokenRepo, apiKeyRepo, passwordResetTokenRepo, emailVerificationTokenRepo, sessionRepo, nil, false, nil, newSecret, oldSecret)
