...
const (
	wsID1  = "01935d5a-0000-7000-8000-000000000001"
	wsID2  = "01935d5a-0000-7000-8000-000000000002"
	uID0   = ""
	uID10  = "01935d5a-0000-7000-8000-00000000000a"
	uID42  = "01935d5a-0000-7000-8000-00000000002a"
	uID99  = "01935d5a-0000-7000-8000-000000000063"
	chID0  = ""
	chID1  = "01935d5a-0000-7000-8000-0000000000c1"
	wsID999 = "01935d5a-0000-7000-8000-0000000003e7"
	nfID99999 = "01935d5a-0000-7000-8000-00000001869f"
)
...
func (m *mockWorkspaceLister) FindWorkspaceIDsByUserID(_ context.Context, _ string) ([]string, error) {
...
func newServiceWithWorkspaces(db *gorm.DB, wsIDs []string) *notifications.Service {
...
func createTestChannel(t *testing.T, svc *notifications.Service, workspaceID string, name string, chanType entity.NotificationChannelType, config string) *entity.NotificationChannel {
	t.Helper()
...
// the test immediately if an error occurs.
func createTestRule(t *testing.T, svc *notifications.Service, workspaceID, channelID string, severity string) *entity.NotificationRule {
	t.Helper()
...
func seedNotification(t *testing.T, db *gorm.DB, workspaceID, userID, channelID string, title, message string, isRead bool) *persistent.Notification {
	t.Helper()
...
func TestCreateChannel_Email(t *testing.T) {
	...
	ch, err := svc.CreateChannel(wsID1, "Email Alerts", entity.NotificationChannelEmail, `{"to":"ops@example.com"}`)
	require.NoError(t, err)
	assert.NotZero(t, ch.ID)
	assert.Equal(t, wsID1, ch.WorkspaceID)
...
func TestCreateChannel_Slack(t *testing.T) {
	...
	ch, err := svc.CreateChannel(wsID1,"Slack Ops", entity.NotificationChannelSlack, `{"webhook":"https://hooks.slack.com/xxx"}`)
	require.NoError(t, err)
	assert.NotZero(t, ch.ID)
...
func TestCreateChannel_Webhook(t *testing.T) {
	...
	ch, err := svc.CreateChannel(wsID1,"Webhook", entity.NotificationChannelWebhook, `{"url":"https://example.com/hook"}`)
	require.NoError(t, err)
	assert.NotZero(t, ch.ID)
...
func TestListChannels_Empty(t *testing.T) {
	...
	channels, err := svc.ListChannels(wsID1)
	require.NoError(t, err)
...
func TestListChannels_ReturnsOnlyOwnOrg(t *testing.T) {
	...
	createTestChannel(t, svc, wsID1,"Org1 Email", entity.NotificationChannelEmail, `{}`)
	createTestChannel(t, svc, wsID1,"Org1 Slack", entity.NotificationChannelSlack, `{}`)
	createTestChannel(t, svc, wsID2,"Org2 Email", entity.NotificationChannelEmail, `{}`)
	...
	channels2, err := svc.ListChannels(wsID2)
	require.NoError(t, err)
...
func TestUpdateChannel_Success(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Original", entity.NotificationChannelEmail, `{"to":"old@example.com"}`)

	updated, err := svc.UpdateChannel(ch.ID, wsID1,"Renamed", `{"to":"new@example.com"}`, false)
	require.NoError(t, err)
...
func TestUpdateChannel_WrongOrg(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Org1 Only", entity.NotificationChannelEmail, `{}`)

	_, err := svc.UpdateChannel(ch.ID, wsID999,"Hacked", `{}`, true)
	require.Error(t, err)
...
func TestUpdateChannel_NotFound(t *testing.T) {
	...
	_, err := svc.UpdateChannel(nfID99999, wsID1,"Ghost", `{}`, true)
	require.Error(t, err)
}
...
func TestDeleteChannel_Success(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"To Delete", entity.NotificationChannelEmail, `{}`)

	err := svc.DeleteChannel(ch.ID, wsID1)
	require.NoError(t, err)
...
func TestDeleteChannel_WrongOrg(t *testing.T) {
	...
	err := svc.DeleteChannel(ch.ID, wsID999)
	require.Error(t, err)
...
func TestDeleteChannel_NotFound(t *testing.T) {
	...
	err := svc.DeleteChannel(nfID99999, wsID1)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "notification channel not found")
}
...
func TestCreateRule_Success(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Alerts", entity.NotificationChannelEmail, `{}`)

	rule, err := svc.CreateRule(wsID1, ch.ID, "high")
	require.NoError(t, err)
	assert.NotZero(t, rule.ID)
	assert.Equal(t, wsID1, rule.WorkspaceID)
...
func TestCreateRule_AllSeverities(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"All Levels", entity.NotificationChannelSlack, `{}`)

	severities := []string{"low", "medium", "high", "critical"}
	for _, sev := range severities {
		rule, err := svc.CreateRule(wsID1, ch.ID, sev)
...
func TestListRules_ReturnsOnlyOwnOrg(t *testing.T) {
	...
	ch1 := createTestChannel(t, svc, wsID1,"Ch1", entity.NotificationChannelEmail, `{}`)
	ch2 := createTestChannel(t, svc, wsID2,"Ch2", entity.NotificationChannelEmail, `{}`)

	createTestRule(t, svc, wsID1,ch1.ID, "high")
	createTestRule(t, svc, wsID1,ch1.ID, "critical")
	createTestRule(t, svc, wsID2,ch2.ID, "low")

	rules, err := svc.ListRules(wsID1)
	require.NoError(t, err)
	assert.Len(t, rules, 2)

	rules2, err := svc.ListRules(wsID2)
	require.NoError(t, err)
...
func TestDeleteRule_Success(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Ch", entity.NotificationChannelEmail, `{}`)
	rule := createTestRule(t, svc, wsID1,ch.ID, "medium")

	err := svc.DeleteRule(rule.ID, wsID1)
	require.NoError(t, err)
...
func TestDeleteRule_WrongOrg(t *testing.T) {
	...
	err := svc.DeleteRule(rule.ID, wsID999)
	require.Error(t, err)
...
func TestDeleteRule_NotFound(t *testing.T) {
	...
	err := svc.DeleteRule(nfID99999, wsID1)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "notification rule not found")
}
...
func TestDispatch_MatchesSeverityAtThreshold(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Email", entity.NotificationChannelEmail, `{}`)
	createTestRule(t, svc, wsID1,ch.ID, "high")

	svc.Dispatch(context.Background(), wsID1,"high", "Alert", "This is a high alert")

	var count int64
	db.Model(&persistent.Notification{}).Where("workspace_id = ?", wsID1).Count(&count)
...
func TestDispatch_MatchesSeverityAboveThreshold(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Email", entity.NotificationChannelEmail, `{}`)
	createTestRule(t, svc, wsID1,ch.ID, "medium")

	svc.Dispatch(context.Background(), wsID1,"critical", "Critical Issue", "Something terrible happened")
...
func TestDispatch_SkipsBelowThreshold(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Email", entity.NotificationChannelEmail, `{}`)
	createTestRule(t, svc, wsID1,ch.ID, "critical")

	svc.Dispatch(context.Background(), wsID1,"low", "Info", "Low priority event")
	...
	db.Where("workspace_id = ?", wsID1).First(&notif)
	assert.Equal(t, chID0, notif.ChannelID, "in-app notification should have ChannelID=0")
}
...
func TestDispatch_MultipleRulesMultipleChannels(t *testing.T) {
	...
	ch1 := createTestChannel(t, svc, wsID1,"Slack Low", entity.NotificationChannelSlack, `{}`)
	ch2 := createTestChannel(t, svc, wsID1,"Email High", entity.NotificationChannelEmail, `{}`)
	ch3 := createTestChannel(t, svc, wsID1,"Webhook Critical", entity.NotificationChannelWebhook, `{"url":"http://invalid.test"}`)

	createTestRule(t, svc, wsID1,ch1.ID, "low")
	createTestRule(t, svc, wsID1,ch2.ID, "high")
	createTestRule(t, svc, wsID1,ch3.ID, "critical")

	svc.Dispatch(context.Background(), wsID1,"high", "High Alert", "Something important")
	...
	db.Where("workspace_id = ?", wsID1).Find(&notifs)
	assert.Len(t, notifs, 1, "expected 1 in-app notification record")
	assert.Equal(t, chID0, notifs[0].ChannelID, "in-app notification should have ChannelID=0")
...
func TestDispatch_AllSeverityLevels(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"All", entity.NotificationChannelEmail, `{}`)
	createTestRule(t, svc, wsID1,ch.ID, "low")

	levels := []string{"low", "medium", "high", "critical"}
	for _, level := range levels {
		svc.Dispatch(context.Background(), wsID1,level, fmt.Sprintf("Title %s", level), "msg")
	}
...
func TestDispatch_NoMatchingRules(t *testing.T) {
	...
	svc.Dispatch(context.Background(), wsID1,"critical", "Nobody Listening", "No rules")
...
func TestDispatch_SkipsDisabledChannel(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Disabled", entity.NotificationChannelEmail, `{}`)
	createTestRule(t, svc, wsID1,ch.ID, "low")

	_, err := svc.UpdateChannel(ch.ID, wsID1,ch.Name, ch.Config, false)
	require.NoError(t, err)

	svc.Dispatch(context.Background(), wsID1,"critical", "Won't Arrive", "Channel is disabled")
...
func TestDispatch_SkipsInactiveRule(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Active Channel", entity.NotificationChannelEmail, `{}`)
	rule := createTestRule(t, svc, wsID1,ch.ID, "low")

	db.Model(&persistent.NotificationRule{}).Where("id = ?", rule.ID).Update("is_active", false)

	svc.Dispatch(context.Background(), wsID1,"critical", "Won't Arrive", "Rule is inactive")
...
func TestDispatch_IsolatedByOrg(t *testing.T) {
	...
	ch1 := createTestChannel(t, svc, wsID1,"Workspace1", entity.NotificationChannelEmail, `{}`)
	ch2 := createTestChannel(t, svc, wsID2,"Org2", entity.NotificationChannelEmail, `{}`)
	createTestRule(t, svc, wsID1,ch1.ID, "low")
	createTestRule(t, svc, wsID2,ch2.ID, "low")

	svc.Dispatch(context.Background(), wsID1,"high", "Org1 Alert", "only org 1")

	var count1, count2 int64
	db.Model(&persistent.Notification{}).Where("workspace_id = ?", wsID1).Count(&count1)
	db.Model(&persistent.Notification{}).Where("workspace_id = ?", wsID2).Count(&count2)
...
func TestDispatch_NotificationFields(t *testing.T) {
	...
	svc.Dispatch(context.Background(), wsID1,"critical", "My Title", "My Message Body")

	var notif persistent.Notification
	require.NoError(t, db.Where("workspace_id = ?", wsID1).First(&notif).Error)
	assert.Equal(t, wsID1, notif.WorkspaceID)
	assert.Equal(t, uID0, notif.UserID, "Dispatch creates org-wide notifications with user_id=0")
	assert.Equal(t, chID0, notif.ChannelID, "in-app notification should have ChannelID=0")
...
func TestDispatch_WebhookDelivery(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Hook", entity.NotificationChannelWebhook, config)
	createTestRule(t, svc, wsID1,ch.ID, "low")

	svc.Dispatch(context.Background(), wsID1,"high", "Webhook Title", "Webhook Body")
...
func TestDispatch_WebhookToUnreachableURL(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Dead Hook", entity.NotificationChannelWebhook, `{"url":"http://127.0.0.1:1"}`)
	createTestRule(t, svc, wsID1,ch.ID, "low")

	svc.Dispatch(context.Background(), wsID1,"critical", "Unreachable", "The webhook URL is dead")
...
func TestDispatch_WebhookInvalidConfig(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Bad Config", entity.NotificationChannelWebhook, `not-json`)
	createTestRule(t, svc, wsID1,ch.ID, "low")

	svc.Dispatch(context.Background(), wsID1,"critical", "Bad Config", "Config is not valid JSON")
...
func TestDispatch_WebhookEmptyURL(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Empty URL", entity.NotificationChannelWebhook, `{"url":""}`)
	createTestRule(t, svc, wsID1,ch.ID, "low")

	svc.Dispatch(context.Background(), wsID1,"critical", "Empty URL", "URL field is empty")
...
func TestDispatch_WebhookServerError(t *testing.T) {
	...
	config := fmt.Sprintf(`{"url":"%s"}`, ts.URL)
	ch := createTestChannel(t, svc, wsID1,"Error Hook", entity.NotificationChannelWebhook, config)
	createTestRule(t, svc, wsID1,ch.ID, "low")

	svc.Dispatch(context.Background(), wsID1,"high", "Server Error", "The hook returned 500")
...
func TestListNotifications_Empty(t *testing.T) {
	...
	notifs, err := svc.ListNotifications(wsID1, uID10,false)
	require.NoError(t, err)
...
func TestListNotifications_OrgWideVisibleToUser(t *testing.T) {
	...
	seedNotification(t, db, wsID1, uID0, chID1,"Org Alert", "For everyone", false)

	notifs, err := svc.ListNotifications(wsID1, uID42,false)
	require.NoError(t, err)
...
func TestListNotifications_UserSpecificPlusOrgWide(t *testing.T) {
	...
	seedNotification(t, db, wsID1, uID0, chID1,"Org", "For everyone", false)
	seedNotification(t, db, wsID1, uID42, chID1,"User 42", "Only for user 42", false)
	seedNotification(t, db, wsID1, uID99, chID1,"User 99", "Only for user 99", false)
...
func TestListNotifications_OnlyUnread(t *testing.T) {
	...
	seedNotification(t, db, wsID1, uID0, chID1,"Read", "Already read", true)
	seedNotification(t, db, wsID1, uID0, chID1,"Unread", "Not yet read", false)

	notifs, err := svc.ListNotifications(wsID1, uID42,true)
	require.NoError(t, err)
...
func TestListNotifications_FilterByOrg(t *testing.T) {
	...
	seedNotification(t, db, wsID1, uID0, chID1,"Workspace1", "from org 1", false)
	seedNotification(t, db, wsID2, uID0, chID1,"Org2", "from org 2", false)
...
func TestListNotifications_ZeroOrgShowsAll(t *testing.T) {
	...
	svc := newServiceWithWorkspaces(db, []string{wsID1, wsID2})

	seedNotification(t, db, wsID1, uID0, chID1,"Workspace1", "from org 1", false)
	seedNotification(t, db, wsID2, uID0, chID1,"Org2", "from org 2", false)

	notifs, err := svc.ListNotifications("", uID42,false)
	require.NoError(t, err)
	assert.Len(t, notifs, 2)
}
...
func TestListNotifications_OrderedByCreatedAtDesc(t *testing.T) {
	...
	seedNotification(t, db, wsID1, uID0, chID1,"First", "created first", false)
	time.Sleep(10 * time.Millisecond)
	seedNotification(t, db, wsID1, uID0, chID1,"Second", "created second", false)
...
func TestMarkRead_Success(t *testing.T) {
	db := setupTestDB(t)
	svc := newServiceWithWorkspaces(db, []string{wsID1})

	n := seedNotification(t, db, wsID1, uID42, chID1,"Unread", "please read me", false)

	err := svc.MarkRead(n.ID, uID42)
	require.NoError(t, err)
...
func TestMarkRead_OrgWideNotification(t *testing.T) {
	...
	n := seedNotification(t, db, wsID1, uID0, chID1,"Org-Wide", "for everyone", false)
...
func TestMarkRead_NotFound(t *testing.T) {
	...
	err := svc.MarkRead(nfID99999, uID42)
	require.Error(t, err)
...
func TestMarkRead_WrongUser(t *testing.T) {
	...
	n := seedNotification(t, db, wsID1, uID42, chID1,"Private", "only user 42", false)

	err := svc.MarkRead(n.ID, uID99)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "notification not found")
}
...
func TestGetUnreadCount_AllUnread(t *testing.T) {
	...
	seedNotification(t, db, wsID1, uID0, chID1,"N1", "m1", false)
	seedNotification(t, db, wsID1, uID0, chID1,"N2", "m2", false)
	seedNotification(t, db, wsID1, uID42, chID1,"N3", "m3", false)

	count, err := svc.GetUnreadCount(wsID1, uID42)
	require.NoError(t, err)
...
func TestGetUnreadCount_ExcludesRead(t *testing.T) {
	...
	seedNotification(t, db, wsID1, uID0, chID1,"Read", "m", true)
	seedNotification(t, db, wsID1, uID0, chID1,"Unread", "m", false)
...
func TestGetUnreadCount_ExcludesOtherUserNotifications(t *testing.T) {
	...
	seedNotification(t, db, wsID1, uID99, chID1,"Other User", "not mine", false)
...
func TestGetUnreadCount_FilterByOrg(t *testing.T) {
	...
	seedNotification(t, db, wsID1, uID0, chID1,"Workspace1", "m", false)
	seedNotification(t, db, wsID2, uID0, chID1,"Org2", "m", false)
...
func TestGetUnreadCount_ZeroOrgCountsAll(t *testing.T) {
	...
	count, err := svc.GetUnreadCount("", uID42)
	require.NoError(t, err)
...
func TestFullNotificationLifecycle(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Lifecycle Email", entity.NotificationChannelEmail, `{}`)
	createTestRule(t, svc, wsID1,ch.ID, "low")

	svc.Dispatch(context.Background(), wsID1,"critical", "Lifecycle Test", "Testing the full flow")
	...
	err = svc.MarkRead(notifs[0].ID, uID10)
	require.NoError(t, err)
	...
	unread, err := svc.ListNotifications(wsID1, uID10,true)
	require.NoError(t, err)
	assert.Empty(t, unread)

	all, err := svc.ListNotifications(wsID1, uID10,false)
	require.NoError(t, err)
	assert.Len(t, all, 1)
...
func TestDispatch_WebhookHMAC_SignaturePresent(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"HMAC Hook", entity.NotificationChannelWebhook, config)
	createTestRule(t, svc, wsID1,ch.ID, "low")

	svc.Dispatch(context.Background(), wsID1,"high", "Signed Title", "Signed Body")
...
func TestDispatch_WebhookHMAC_NoSecretNoHeader(t *testing.T) {
	...
	config := fmt.Sprintf(`{"url":"%s"}`, ts.URL)
	ch := createTestChannel(t, svc, wsID1,"No Secret Hook", entity.NotificationChannelWebhook, config)
	createTestRule(t, svc, wsID1,ch.ID, "low")

	svc.Dispatch(context.Background(), wsID1,"high", "Unsigned Title", "Unsigned Body")
...
func TestDispatch_WebhookHMAC_EmptySecretNoHeader(t *testing.T) {
	...
	ch := createTestChannel(t, svc, wsID1,"Empty Secret", entity.NotificationChannelWebhook, config)
	createTestRule(t, svc, wsID1,ch.ID, "low")

	svc.Dispatch(context.Background(), wsID1,"high", "Test", "Test Body")
...
func TestDispatch_WebhookHMAC_DifferentSecretsProduceDifferentSignatures(t *testing.T) {
	...
	ch1 := createTestChannel(t, svc, wsID1,"Hook A", entity.NotificationChannelWebhook, config1)
	createTestRule(t, svc, wsID1,ch1.ID, "low")

	config2 := fmt.Sprintf(`{"url":"%s","secret":"secret-beta"}`, ts.URL)
	ch2 := createTestChannel(t, svc, wsID2,"Hook B", entity.NotificationChannelWebhook, config2)
	createTestRule(t, svc, wsID2,ch2.ID, "low")

	svc.Dispatch(context.Background(), wsID1,"high", "Same Title", "Same Body")
	svc.Dispatch(context.Background(), wsID2,"high", "Same Title", "Same Body")
