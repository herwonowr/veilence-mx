...
type DifferRepository interface {
	...
	FindReleaseByIDWithPackage(ctx context.Context, id string) (*entity.Release, *entity.Package, error)
	// FindPreviousCompletedRelease finds the latest completed release before the given one.
	FindPreviousCompletedRelease(ctx context.Context, packageID string, beforePublishedAt time.Time) (*entity.Release, error)
	...
	UpdateReleaseError(ctx context.Context, id string, status entity.ReleaseStatus, errorMessage string) error
