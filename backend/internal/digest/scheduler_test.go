...
func TestGenerateDigest_WithData(t *testing.T) {
	...
	alert1 := models.Alert{AnalysisID: analysis.ID, PackageID: pkg.ID, OrgID: 1, Severity: "critical", Status: "new", Message: "Critical alert"}
	db.Create(&alert1)
	alert2 := models.Alert{AnalysisID: analysis.ID, PackageID: pkg.ID, OrgID: 1, Severity: "medium", Status: "new", Message: "Medium alert"}
	db.Create(&alert2)
...
func TestGenerateDigest_OrgScoping(t *testing.T) {
	...
	db.Create(&rel1)
	rel2 := models.Release{PackageID: pkg2.ID, Version: "1.0.0", Status: "completed"}
	db.Create(&rel2)

	diff1 := models.Diff{ReleaseID: rel1.ID, DiffContent: "d1"}
	db.Create(&diff1)
	diff2 := models.Diff{ReleaseID: rel2.ID, DiffContent: "d2"}
	db.Create(&diff2)

	analysis1 := models.Analysis{DiffID: diff1.ID, Classification: "malicious", Confidence: 0.9, ModelUsed: "test", AnalyzerType: "copilot"}
	db.Create(&analysis1)
	analysis2 := models.Analysis{DiffID: diff2.ID, Classification: "benign", Confidence: 0.95, ModelUsed: "test", AnalyzerType: "copilot"}
	db.Create(&analysis2)

	db.Create(&models.Alert{AnalysisID: analysis1.ID, PackageID: pkg1.ID, OrgID: 1, Severity: "critical", Status: "new", Message: "Org 1 alert"})
	db.Create(&models.Alert{AnalysisID: analysis2.ID, PackageID: pkg2.ID, OrgID: 2, Severity: "low", Status: "new", Message: "Org 2 alert"})
...
func TestGenerateDigest_TopAlertsLimit(t *testing.T) {
	...
	db.Create(&diff)
	analysis := models.Analysis{DiffID: diff.ID, Classification: "malicious", Confidence: 0.9, ModelUsed: "test", AnalyzerType: "copilot"}
	db.Create(&analysis)
	...
	for i := range 7 {
		...
		db.Create(&models.Alert{AnalysisID: analysis.ID, PackageID: pkg.ID, OrgID: 1, Severity: models.AlertSeverity(severity), Status: "new", Message: "Alert"})
	}
