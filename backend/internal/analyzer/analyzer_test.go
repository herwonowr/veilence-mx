...
func seedDiffForPipeline(t *testing.T, db *gorm.DB) (models.Diff, models.Release, models.Release) {
	...
	rel2 := models.Release{PackageID: pkg.ID, Version: "1.1.0", Status: models.ReleaseStatusAnalyzing}
...
func TestPipeline_ProcessDiff_Benign(t *testing.T) {
	...
	assert.Equal(t, models.ClassificationBenign, analysis.Classification)
	...
	assert.Equal(t, models.ReleaseStatusCompleted, updated.Status)
}
...
func TestPipeline_ProcessDiff_Suspicious(t *testing.T) {
	...
	var alert models.Alert
	require.NoError(t, db.First(&alert).Error)
	assert.Equal(t, models.AlertSeverityMedium, alert.Severity)
}
