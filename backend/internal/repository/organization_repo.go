...
// OrganizationRepo implements domain.OrganizationRepository using GORM.
...
func (r *OrganizationRepo) FindByID(ctx context.Context, id uint) (*domain.Organization, error) {
	var m models.Organization
	if err := r.db.WithContext(ctx).First(&m, id).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, fmt.Errorf("organization %w", domain.ErrNotFound)
		}
...
func (r *OrganizationRepo) CountBySlug(ctx context.Context, slug string, excludeID *uint) (int64, error) {
	query := r.db.WithContext(ctx).Model(&models.Organization{}).Where("slug = ?", slug)
	if excludeID != nil {
...
func (r *OrganizationRepo) SoftDelete(ctx context.Context, id uint) error {
	...
	if result.RowsAffected == 0 {
		return fmt.Errorf("organization %w", domain.ErrNotFound)
	}
	return nil
}

func (r *OrganizationRepo) FindByUserID(ctx context.Context, userID uint) ([]domain.Organization, error) {
	var ms []models.Organization
	...
	result := make([]domain.Organization, len(ms))
	for i := range ms {
...
func orgToDomain(m *models.Organization) *domain.Organization {
...
func orgToModel(d *domain.Organization) *models.Organization {
