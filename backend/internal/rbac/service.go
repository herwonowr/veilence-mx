...
func (s *Service) CreateOrganization(userID uint, name, slug, description string) (*models.Organization, error) {
	// Check slug uniqueness
	var count int64
	s.db.Model(&models.Organization{}).Where("slug = ?", slug).Count(&count)
	if count > 0 {
...
func (s *Service) createDefaultRoles(tx *gorm.DB, orgID uint) ([]models.Role, error) {
	...
	lookupPerms := func(keys []string) []models.Permission {
	...
	roleDefinitions := []struct {
		...
		{models.RoleOwner, "Full access to the organization", allKeys},
		{models.RoleAdmin, "Administrative access (cannot delete organization)", adminKeys},
		{models.RoleMember, "Standard member with read/write access", memberKeys},
		{models.RoleViewer, "Read-only access", viewerKeys},
	}
...
func (s *Service) GetUserOrganizations(userID uint) ([]models.Organization, error) {
...
func (s *Service) GetOrganization(orgID uint) (*models.Organization, error) {
...
func (s *Service) UpdateOrganization(orgID uint, name, slug, description string) (*models.Organization, error) {
	var org models.Organization
	if err := s.db.First(&org, orgID).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrOrgNotFound
	...
	if slug != org.Slug {
		var count int64
		s.db.Model(&models.Organization{}).Where("slug = ? AND id != ?", slug, orgID).Count(&count)
		if count > 0 {
			return nil, ErrSlugTaken
		}
	}
...
func (s *Service) DeleteOrganization(orgID uint) error {
	result := s.db.Delete(&models.Organization{}, orgID)
	if result.Error != nil {
...
func (s *Service) InviteMember(orgID uint, email string, roleID, invitedBy uint) (*models.Invitation, string, error) {
...
func (s *Service) AcceptInvitation(token string, userID uint, userEmail string) (*models.OrgMember, error) {
...
func (s *Service) GetInvitationByToken(token string) (*models.Invitation, error) {
	var invitation models.Invitation
	if err := s.db.Where("token_hash = ?", hashToken(token)).First(&invitation).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrInvitationNotFound
...
func (s *Service) ListPendingInvitations(orgID uint) ([]models.Invitation, error) {
...
func (s *Service) UpdateMemberRole(orgID, userID, newRoleID uint) (*models.OrgMember, error) {
	// Check if user is the owner
	var org models.Organization
	if err := s.db.First(&org, orgID).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
...
func (s *Service) GetOrgRoles(orgID uint) ([]models.Role, error) {
	var roles []models.Role
	err := s.db.
		Preload("Permissions").
		Where("org_id = ?", orgID).
...
func (s *Service) GetUserMembership(userID, orgID uint) (*models.OrgMember, error) {
	var member models.OrgMember
	err := s.db.
		Preload("Role").
...
func SeedPermissions(db *gorm.DB) error {
	...
	slog.Info("system permissions seeded", "count", len(models.SystemPermissions))
	return nil
}
